...
class CitySelector extends React.Component<CitySelectorProps, any>{
  ...
  componentDidMount() {
    ...
    if (defaultValue && this.state.cities.length === 0) {
      ...
      let province: any
      province = provinces.filter((cur: any) => cur.name === provinceName)[0] || {}
  ...
  componentWillReceiveProps(nextProps: any) {
    ...
    if (defaultValue && this.state.cities.length === 0) {
      const { provinceName } = defaultValue
      const province = provinces.filter((cur: any) => cur.name === provinceName)[0] || {}
      const cities = province.cityInfo || []
      this.setState({
        provinces,
        cities,
  ...
  handleProvinceChange = (value: any) => {
    ...
    const province = provinces.filter((cur: any) => cur.name === value)[0] || {}
    const cities = province.cityInfo || []
    const city = cities[0] || {}
    setFieldsValue({
      cityName: city.name,
            ...
            >
              {
                provinces.map((cur: any) => (
                  <Select.Option value={cur.name} key={cur.id}>{cur.name}</Select.Option>
                ))
              }
            </Select>,
          )}
